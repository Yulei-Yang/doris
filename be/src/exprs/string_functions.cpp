...
StringVal StringFunctions::substring_index(FunctionContext* context, const StringVal& content,
                                           const StringVal& delimiter, const IntVal& field) {
    if (content.is_null || delimiter.is_null || field.is_null) {
        return StringVal::null();
    }
    ...
    // if field not found or field is over-long, then return the whole string
    if (split_cnt == 0 || field.val >= split_cnt || (field.val < 0 && new_field <= 1)) {
        return content;
    }
    ...
    if (field.val > 0) {
        start_pos = 0;
        len = find[new_field - 1];
    } else {
        start_pos = find[new_field - 2] + delimiter.len;
        len = content.len - start_pos;
    }
    return StringVal(content.ptr + start_pos, len);
}
